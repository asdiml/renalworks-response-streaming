...
custom functions in chains (in technical terms, chains are called Runnables in langchain)

parse() is now a generator
'''
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Generator[None, Any, None]:
    ...
    # This check will not fail because the pydantic Event model alr does a check of the request body
    if response_data is None or category is None:
        print("`responseData` and `category` fields are required")
        return "`responseData` and `category` fields are required"

    if not context_limiter(formatted_response_data):
        print("Context Length reached")
        return "Context Length reached"
    
    user_input = event.userInput
    previous_summary = event.previousSummary
